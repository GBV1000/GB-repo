...
class Cell:
	...
	def __str__(self):
		return (f'В наличии {self.quantity} cells')

	def __add__(self, other):
		return (f'Сумма cells: {self.quantity + other.quantity}')

	def __sub__(self, other):
		if self.quantity >= other.quantity:
			return (f'Разница cells  равна: {self.quantity - other.quantity}')
		else:
			return (f'Остаток не более 0')

	def __mul__(self, other):
		return (f'Результат умножения : {self.quantity * other.quantity}')

	def __floordiv__(self, other):
		return (f'Результат целочисленного деления: {self.quantity // other.quantity}')

	def __truediv__(self, other):
		return (f'Результат математического деления с остатком после запятой: {self.quantity / other.quantity}')

	def make_order(self, cells_quantity_in_row):
		return '\\n'.join(['*' * cells_quantity_in_row for _ in range(self.quantity // cells_quantity_in_row)]) + "\\n" + '*' * (self.quantity % cells_quantity_in_row)
